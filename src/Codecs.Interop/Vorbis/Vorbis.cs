...
namespace Codecs.Interop.Vorbis
{
    ...
    public abstract class Vorbis
    {
        ...
        internal const string DllFile = @"x64/libvorbis.dll";
        ...
        /// <returns></returns>
        public static IntPtr AnalysisBuffer(VorbisDspState v, int vals)
        {
            if (v == null)
                throw new ArgumentException("VorbisDspState v cannot be null!");

            IntPtr pbuffer = vorbis_analysis_buffer(ref v.vorbis_dsp_state, vals);
            v.changed = true;
            //VorbisInfo vi = v.Vi;
            //float[][] buffer = new float[vi.Channels][];
            //int[] temp = new int[vi.Channels];
            //Marshal.Copy(pbuffer, temp, 0, vi.Channels);
            //for (int i = 0; i < vi.Channels; i++)
            //{
            //    IntPtr ptr = (IntPtr)temp[i];
            //    buffer[i] = new float[v.PcmStorage - v.PcmCurrent];
            //    Marshal.Copy(ptr, buffer[i], 0, v.PcmStorage - v.PcmCurrent);
            //}
            //return buffer;
        ...
        [DllImport(Vorbis.DllFile, CallingConvention = CallingConvention.Cdecl)]
        private static extern IntPtr vorbis_analysis_buffer(IntPtr v, int vals);
        ...
        /// <param name="vals"></param>
        /// <returns></returns>
        public static IntPtr AnalysisBuffer(IntPtr dspState, int vals)
        {
            if (dspState == IntPtr.Zero)
                throw new ArgumentException("VorbisDspState v cannot be null!");

            return vorbis_analysis_buffer(dspState, vals);
        }
        ...
        [DllImport(Vorbis.DllFile, CallingConvention = CallingConvention.Cdecl)]
        private static extern int vorbis_analysis_wrote(IntPtr v, int vals);
        ...
        /// <returns></returns>
        public static int AnalysisWrote(IntPtr dspState, int samples)
        {
            if (dspState == IntPtr.Zero)
                throw new ArgumentException("VorbisDspState v cannot be null!");

            return vorbis_analysis_wrote(dspState, samples);
        }
        ...
        [DllImport(Vorbis.DllFile, CallingConvention = CallingConvention.Cdecl)]
        private static extern int vorbis_analysis_blockout(IntPtr v, IntPtr vb);
        /// <summary>
        /// </summary>
        /// <param name="v"></param>
        /// <param name="vb"></param>
        ...
        /// <summary>
        /// </summary>
        /// <param name="dspState"></param>
        /// <param name="block"></param>
        /// <returns></returns>
        public static int AnalysisBlockout(IntPtr dspState, IntPtr block)
        {
            if (dspState == IntPtr.Zero)
                throw new ArgumentException("VorbisDspState v cannot be null!");
            ...
            int retVal = 0;
            retVal = vorbis_analysis_blockout(dspState, block);
        ...
        [DllImport(Vorbis.DllFile, CallingConvention = CallingConvention.Cdecl)]
        private static extern int vorbis_analysis(IntPtr vb, [In, Out]	ref ogg_packet op);

        [DllImport(Vorbis.DllFile, CallingConvention = CallingConvention.Cdecl)]
        private static extern int vorbis_analysis(IntPtr vb, IntPtr op);
        /// <summary>
        /// </summary>
        /// <param name="block"></param>
        /// <param name="oggPacket"></param>
        /// <returns></returns>
        public static int Analysis(VorbisBlock block, OggPacket oggPacket)
        {
            if (block == null)
                throw new ArgumentException("VorbisBlock vb cannot be null!");
            int retval;
            if (oggPacket == null)
                retval = vorbis_analysis(ref block.vorbis_block, IntPtr.Zero);
            else
            {
                retval = vorbis_analysis(ref block.vorbis_block, ref oggPacket.ogg_packet);
                oggPacket.changed = true;
            }
            block.changed = true;
            return retval;
        }
        public static int Analysis(IntPtr block, OggPacket oggPacket)
        {
            if (block == IntPtr.Zero)
                throw new ArgumentException("VorbisBlock vb cannot be null!");
            ...
            int retVal;
            if (oggPacket == null)
                retVal = vorbis_analysis(block, IntPtr.Zero);
            else 
            {
                retVal = vorbis_analysis(block, ref oggPacket.ogg_packet);
                oggPacket.changed = true;
            }
    ...
    public abstract class VorbisBitrate
    {
        ...
        public static int Addblock(IntPtr block)
        {
            if (block == IntPtr.Zero)
                throw new ArgumentException("VorbisBlock vb cannot be null!");

            return vorbis_bitrate_addblock(block);
        }
        #endregion
        ...
        public static int Flushpacket(IntPtr block, OggPacket oggPacket)
        {
            if (block == IntPtr.Zero)
                throw new ArgumentException("VorbisDspState vd cannot be null!");
            if (oggPacket == null)
            ...
            int retval = vorbis_bitrate_flushpacket(block, ref oggPacket.ogg_packet);
            oggPacket.changed = true;
            return retval;
        }
        #endregion
