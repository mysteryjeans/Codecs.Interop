...
namespace Codecs.Interop.Vorbis
{
	...
	/// <summary>
	/// </summary>
	public class VorbisBlock : IDisposable
	{
		#region Variables
...
//		}
        ...
        [DllImport(Vorbis.DllFile, CallingConvention = CallingConvention.Cdecl)]
        private static extern int vorbis_block_init(IntPtr v, IntPtr vb);
		...
		/// <param name="vb"></param>
		/// <returns></returns>
		public static int Init(VorbisBlock vb, VorbisDspState vd) 
		{
			if(vb == null)
				throw new ArgumentException("VorbisBlock vb cannot be null!");
            int retval = vorbis_block_init(ref vd.vorbis_dsp_state, ref vb.vorbis_block);
			vb.changed = true;
            vb.initialized = true;
			return retval;
		}
        ...
        /// <returns></returns>
        public static int Init(IntPtr block, IntPtr dspState)
        {
            if (block == IntPtr.Zero)
                throw new ArgumentException("VorbisBlock vb cannot be null!");

            return vorbis_block_init(dspState, block);
        }
		#endregion
